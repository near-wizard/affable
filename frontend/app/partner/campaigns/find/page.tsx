...
function CampaignCard({ campaign }: CampaignCardProps) {
  ...
  const handleApply = async () => {
    try {
      setApplyError(null);
      await applyCampaign(campaign.id);
      // Show success - could add a toast notification here
    } catch (error) {
      setApplyError(error instanceof Error ? error.message : 'Failed to apply to campaign');
    }
  };
    ...
    <div className="bg-white rounded-lg shadow hover:shadow-lg transition p-6 flex flex-col h-full">
      ...
      <div className="mb-4">
        <div className="flex items-start justify-between mb-3">
          <div className="flex-1">
            ...
            <p className="text-sm text-gray-600">{campaign.vendor_name || 'Unknown Vendor'}</p>
          </div>
          {campaign.category && (
      ...
      <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
        <div className="flex items-center justify-between">
          ...
          <span className="text-lg font-bold text-blue-600">{commissionDisplay}</span>
        </div>
      </div>

      {/* Stats Grid */}
      {(campaign.partner_count !== undefined || campaign.conversion_count !== undefined || campaign.conversion_rate !== undefined) && (
        <div className="grid grid-cols-3 gap-3 mb-6">
          {campaign.partner_count !== undefined && (
            <div className="text-center p-3 bg-gray-50 rounded-lg">
              <div className="text-xs text-gray-600 mb-1">Partners</div>
              <div className="text-lg font-bold text-gray-900">{campaign.partner_count.toLocaleString()}</div>
            </div>
          )}
          {campaign.conversion_count !== undefined && (
            <div className="text-center p-3 bg-gray-50 rounded-lg">
              <div className="text-xs text-gray-600 mb-1">Conversions</div>
              <div className="text-lg font-bold text-gray-900">{campaign.conversion_count.toLocaleString()}</div>
            </div>
          )}
      ...
      {campaign.total_revenue !== undefined && (
        <div className="mb-6 p-3 bg-green-50 border border-green-200 rounded-lg flex items-center gap-2">
          ...
          <span className="text-sm text-gray-700">
            <span className="font-semibold">${(campaign.total_revenue / 1000000).toFixed(1)}M</span> GMV tracked
          </span>
        </div>
      )}
...
export default function FindCampaignsPage() {
  ...
  const { data: campaignsResponse, loading, error } = useCampaigns({
    page,
    limit: 12,
    search: searchQuery,
      ...
      <div className="bg-white shadow">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div>
            ...
            <p className="text-gray-600 mt-1">Discover and join the best affiliate programs</p>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <>
            ...
            <div className="mb-8 flex items-center justify-between">
              <p className="text-sm text-gray-600">
                Showing <span className="font-semibold text-gray-900">{filteredCampaigns.length}</span> campaign
                {filteredCampaigns.length !== 1 ? 's' : ''}
                {campaignsResponse?.total !== undefined && (
                  <span> of <span className="font-semibold">{campaignsResponse.total}</span> total</span>
                )}
              </p>
            </div>
            ...
            {filteredCampaigns.length > 0 ? (
              <>
                ...
                {/* Pagination */}
                {campaignsResponse?.total_pages && campaignsResponse.total_pages > 1 && (
                  <div className="flex items-center justify-center gap-2 mb-12">
                    ...
                    <span className="text-sm text-gray-600">
                      ...
                      <span className="font-semibold">{campaignsResponse.total_pages}</span>
                    </span>
                    <button
                      onClick={() => setPage(Math.min(campaignsResponse.total_pages!, page + 1))}
        ...
        <div className="grid gap-6 md:grid-cols-3">
          <div className="bg-white rounded-lg shadow p-6">
            ...
            <h3 className="font-semibold text-gray-900 mb-2">Growing Opportunities</h3>
            <p className="text-sm text-gray-600">
          ...
          <div className="bg-white rounded-lg shadow p-6">
            ...
            <h3 className="font-semibold text-gray-900 mb-2">Active Communities</h3>
            <p className="text-sm text-gray-600">
          ...
          <div className="bg-white rounded-lg shadow p-6">
            ...
            <h3 className="font-semibold text-gray-900 mb-2">Competitive Payouts</h3>
            <p className="text-sm text-gray-600">
